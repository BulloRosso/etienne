...
/**
 * Invoke `tsx wiki-add.ts --input <tmp.json>` from `projectRoot`. The skill
 * script reads its input file, writes the page, and emits a single-line
...
function spawnTsx(scriptPath: string, args: string[], cwd: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    // npx.cmd on Windows; shell:true picks it up from PATH.
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        // wiki-add.ts emits JSON {ok:false,...} on user-level errors and
        // exits non-zero. Surface stdout if it parses as JSON, otherwise
        // include stderr in the rejection.
        try {
          JSON.parse(stdout);
          resolve(stdout);
          return;
