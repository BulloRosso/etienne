...
* /frontend contains the React/Vite/Typescript browser frontend running on :5000
...
docker run -it \
  --name claude-code \
  -v C:/Data/GitHub/claude-multitenant/workspace:/workspace \
  -e ANTHROPIC_API_KEY=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 \
  claude-code
```
...
# Switch to root to modify sudoers
USER root

# Allow node user to install Python packages
RUN echo 'node ALL=(ALL) NOPASSWD: /usr/bin/pip, /usr/bin/pip3, /usr/local/bin/pip, /usr/local/bin/pip3' >> /etc/sudoers
