...
export default function DocxViewer({ filename, projectName }) {
  ...
  const loadDocx = async () => {
    try {
      ...
      const response = await apiFetch(`/api/workspace/${encodeURIComponent(projectName)}/preview-docx/${filename}`);
      if (!response.ok) {
        const text = await response.text().catch(() => response.statusText);
        throw new Error(`Failed to convert DOCX: ${text}`);
      }
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{
        ...
        <Tooltip title={t('common.refresh', 'Refresh')}>
          <IconButton size="small" onClick={loadDocx}>
